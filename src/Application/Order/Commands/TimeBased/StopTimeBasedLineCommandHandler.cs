...
public class StopTimeBasedLineCommandHandler : IRequestHandler<StopTimeBasedLineCommand, OrderResponse>
{
    private readonly IOrderLineRepository _orderLineRepository;
    private readonly IOrderRepository _orderRepository;
    private readonly ICurrentUserService _currentUserService;

    public StopTimeBasedLineCommandHandler(
    ...
    public async Task<OrderResponse> Handle(StopTimeBasedLineCommand request, CancellationToken cancellationToken)
    {
        ...
        if (line.TimeBasedStartedAt is null || line.TimeBasedStoppedAt is not null)
            throw new Exception("Taymer başladılmayıb.");

        var stoppedAt = DateTime.UtcNow;
        var elapsedHours = (decimal)(stoppedAt - line.TimeBasedStartedAt.Value).TotalHours;

        line.TimeBasedStoppedAt = stoppedAt;
        line.LineTotal = Math.Round(line.UnitPrice * elapsedHours, 2);
